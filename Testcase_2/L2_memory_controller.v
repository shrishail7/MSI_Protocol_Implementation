...
module L2_memory_controller (
    ...
    output reg mem_req_write,     // ⭐ Write to memory
    ...
    always @(posedge clk) begin
        if(reset) begin
            ...
            // ⭐ NEW: Check for write-through request from P1
            if(p1_mem_wr_req_valid) begin
                mem_req_valid <= 1'b1;
                mem_req_write <= 1'b1;      // ⭐ Write to memory
                mem_req_read <= 1'b0;
                mem_req_addr <= p1_mem_wr_addr;
                mem_req_data <= p1_mem_wr_data;
            end
            else begin
