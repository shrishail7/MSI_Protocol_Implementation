...
parameter STATE_WRITE_MEM = 3'd6;  // ⭐ NEW: Write to memory (Write-Through)
...
if(reset) begin
    ...
    l1_rd_set_idx <= 5'd0;
    mem_wr_req_valid <= 1'b0;  // ⭐ NEW: Initialize
    mem_wr_addr <= 16'b0;      // ⭐ NEW: Initialize
    mem_wr_data <= 128'b0;     // ⭐ NEW: Initialize
end
else begin
    // ========== Default Assignments ==========
    l1_wr_en <= 1'b0;
    bus_req_valid <= 1'b0;
    proc_resp_valid <= 1'b0;
    mem_wr_req_valid <= 1'b0;  // ⭐ NEW: Default to 0
        ...
        if((l1_rd_valid_way0 && (l1_rd_tag_way0 == pending_tag)) ||
           (l1_rd_valid_way1 && (l1_rd_tag_way1 == pending_tag))) begin
            ...
            if(hit_msi_state == `MSI_MODIFIED) begin
                ...
                // ⭐ NEW: Write to memory immediately (Write-Through)
    ...
    STATE_TRANSIENT: begin
        ...
        // ⭐ NEW: If WRITE operation, go to WRITE_MEM state for write-through
        if(pending_req_type == `PR_WR) begin
            current_state <= STATE_WRITE_MEM;
        end
        else begin
    ...
    // ⭐ NEW STATE: Write to main memory (Write-Through Protocol)
    STATE_WRITE_MEM: begin
        l1_rd_set_idx <= pending_set_idx;
        
        mem_wr_req_valid <= 1'b1;    // ⭐ Assert write request
        mem_wr_addr <= pending_addr;  // ⭐ Send address
        mem_wr_data <= pending_data;  // ⭐ Send data
        
        // After asserting write, return to idle
        latency_active <= 1'b1;
        latency_counter <= 4'd4;
        proc_resp_hit <= 1'b0;
        current_state <= STATE_IDLE;
