...
assign p2_pending_msi_debug = target_msi_state; // ⭐ FIX: Show target state for debug
...
STATE_IDLE: begin
  if(proc_req_type != `PR_IDLE) begin
    ...
    l1_rd_set_idx <= proc_req_addr[8:4]; // ⭐ FIX: Set read index immediately
...
STATE_BUS_REQ: begin
  l1_rd_set_idx <= pending_set_idx; // ⭐ FIX: Keep read index set
...
STATE_WAIT_L2: begin
  ...
  if(l2_resp_valid) begin
    // FIRST WRITE: ISD state with L2 data (I -> ISD)
    l1_wr_en <= 1'b1;
    l1_wr_set_idx <= pending_set_idx;
    ...
    if(pending_req_type == `PR_RD) begin
      l1_wr_msi <= `MSI_ISD; // 3'd3 - INTERMEDIATE STATE
      l1_wr_dirty <= 1'b0;
      target_msi_state <= `MSI_SHARED; // Final target is S
    end
    else begin
...
STATE_TRANSIENT: begin
  l1_rd_set_idx <= pending_set_idx; // ⭐ FIX: Keep read index set
  // SECOND WRITE: Final MSI state (ISD -> S)
  l1_wr_en <= 1'b1;
  l1_wr_set_idx <= pending_set_idx;
  l1_wr_way <= pending_wr_way; // ⭐ S->M FIX
  l1_wr_tag <= pending_tag;
  ...
  l1_wr_valid <= 1'b1;
  l1_wr_msi <= target_msi_state; // Final state: S(001) for read
  l1_wr_lru <= ~l1_rd_lru_bit; // ⭐ LRU FIX
