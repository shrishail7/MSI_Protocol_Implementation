...
module msi_cache_hierarchy (
    ...
    wire mem_req_valid_sig;
    wire mem_req_read_sig;
    wire mem_req_write_sig;
    wire [15:0] mem_req_addr_sig;
    wire [127:0] mem_req_data_sig;
    ...
    // ⭐ FIXED: Memory debug signal assignments
    assign mem_read = mem_req_read_sig;
    assign mem_write = mem_req_write_sig;
    assign mem_addr = mem_req_addr_sig;
    assign mem_wdata = mem_req_data_sig;
    ...
    L2_memory_controller L2_MEM_CTRL (
        ...
        .l2_wr_lru(l2_wr_lru),
        // ⭐ FIXED: Correct memory ports
        .mem_req_valid(mem_req_valid_sig), 
        .mem_req_read(mem_req_read_sig), 
        .mem_req_write(mem_req_write_sig),
        .mem_req_addr(mem_req_addr_sig),
        .mem_req_data(mem_req_data_sig), 
    ...
    L1_cache #(.No_Sets(32), .No_Ways(2), .Line_Bytes(16), .Tag_Bits(7)) P1_L1 (
        .clk(clk), .reset(reset),
        .rd_set_idx(p1_l1_rd_set_idx), .rd_en(1'b1),
    ...
    L1_cache #(.No_Sets(32), .No_Ways(2), .Line_Bytes(16), .Tag_Bits(7)) P2_L1 (
        .clk(clk), .reset(reset),
        .rd_set_idx(p2_l1_rd_set_idx), .rd_en(1'b1),
    ...
    L2_cache #(.No_Sets(256), .No_Ways(2), .Line_Bytes(16), .Tag_Bits(4)) L2 (
        .clk(clk), .reset(reset),
        .rd_set_index(l2_rd_set_idx), .rd_en(1'b1),
    ...
    main_memory #(.Addr_Width(16), .Line_Bytes(16)) MEMORY (
        .clk(clk), .reset(reset),
        .mem_read(mem_req_read_sig), .mem_write(mem_req_write_sig),
        .mem_addr(mem_req_addr_sig), .mem_wdata(mem_req_data_sig),
