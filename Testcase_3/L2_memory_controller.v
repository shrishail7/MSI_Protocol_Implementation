...
module L2_memory_controller (
    ...
    parameter STATE_MEM_WRITE = 3'd6;  // ⭐ NEW: Handle write-through writes
    ...
    // ⭐ NEW: Write-through control signals
    reg wt_pending;
    reg [15:0] wt_addr;
    reg [127:0] wt_data;
    ...
    always @(posedge clk) begin
        if(reset) begin
            ...
            mem_req_valid <= 1'b0;
            mem_req_read <= 1'b0;
            mem_req_write <= 1'b0;  // ⭐ FIXED: Initialize to 0
            wt_pending <= 1'b0;     // ⭐ NEW
            wt_addr <= 16'b0;       // ⭐ NEW
            wt_data <= 128'b0;      // ⭐ NEW
        end
            ...
            mem_req_valid <= 1'b0;
            mem_req_read <= 1'b0;
            mem_req_write <= 1'b0;  // ⭐ FIXED: Default to 0

            // ⭐ FIXED: Handle write-through requests with priority
            if(p1_mem_wr_req_valid && !wt_pending) begin
                ...
                case(current_state)
                    STATE_MEM_WRITE: begin
                        mem_req_valid <= 1'b1;
                        mem_req_write <= 1'b1;      // ⭐ FIXED: Set write enable
                        mem_req_read <= 1'b0;
                ...
                STATE_L2_MISS: begin
                    ...
                    mem_req_valid <= 1'b1;
                    mem_req_read <= 1'b1;
                    mem_req_write <= 1'b0;  // ⭐ FIXED: Explicitly set to 0
