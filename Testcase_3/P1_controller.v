...
if(reset) begin
    ...
    mem_wr_addr <= 16'b0;
    mem_wr_data <= 128'b0;
    write_through_pending <= 1'b0;  // ⭐ NEW
    wt_addr <= 16'b0;               // ⭐ NEW
    wt_data <= 128'b0;              // ⭐ NEW
end
else begin
    ...
    // ⭐ FIX: Handle write-through requests separately from state machine
        ...
        if((l1_rd_valid_way0 && (l1_rd_tag_way0 == pending_tag)) ||
           (l1_rd_valid_way1 && (l1_rd_tag_way1 == pending_tag))) begin
            ...
            if(hit_msi_state == `MSI_MODIFIED) begin
                ...
                // ⭐ FIX: Trigger write-through immediately with proper timing
    ...
    STATE_TRANSIENT: begin
        ...
        // ⭐ FIX: Trigger write-through for WRITE operations
        if(pending_req_type == `PR_WR) begin
            write_through_pending <= 1'b1;
            wt_addr <= pending_addr;
            wt_data <= pending_data;
    ...
    // ⭐ NOTE: STATE_WRITE_MEM is no longer needed - handled by write_through_pending
    endcase
